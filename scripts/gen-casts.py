...
def gen_sign_demo():
    ...
    e, t = line(f"{BOLD}sign demo{RST}  {DIM}fully offline · local PAdES signer · no signup{RST}", t, 0.4)
    parts.append(e)
    e, t = line(f"{DIM}[sign] resolved provider: local (default — no flag, no SIGN_PROVIDER set){RST}", t, 0.3)
    parts.append(e)
    e, t = line("", t, 0.2)
    parts.append(e)
    e, t = line(f"{CYAN}1.{RST} Creating request {BOLD}contract.pdf{RST} → 2 signers…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} req_a9c4f1  status=draft  hash=sha256:c4f1…1d", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}2.{RST} Issuing per-signer approval tokens…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} alice@acme.com  token=tk_4f…  ttl=60m", t, 0.18)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} bob@beta.com    token=tk_91…  ttl=60m", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}3.{RST} Sending via local provider…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} status=sent  ev_2  hash_prev=c4f1…1d  hash_self=8a52…7e", t, 0.5)
    parts.append(e)
    e, t = line(f"{CYAN}4.{RST} Alice signs (token=tk_4f…) → PAdES PKCS#7…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} ev_3  CN=Alice Anderson <alice@acme.com>  cert sha256:7a3b…", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}5.{RST} Bob signs (token=tk_91…) → PAdES PKCS#7…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} ev_4  CN=Bob Beecher <bob@beta.com>     cert sha256:b51c…", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}6.{RST} Anchoring audit head with RFC 3161 TSA…", t, 0.4)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} TSA reply 200 · anchored at 2026-05-14T14:22:08Z", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}7.{RST} {BOLD}sign request verify-signed-pdf{RST}…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} 2 signatures · /ByteRange digests match · trust=self_signed_local", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}8.{RST} {BOLD}sign audit verify{RST} --request-id req_a9c4f1…", t, 0.3)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} chainValid=true · 4 events · 0 breaks", t, 0.4)
    parts.append(e)
    e, t = line(f"{CYAN}9.{RST} {BOLD}sign request receipt{RST} → bundleVersion=1…", t, 0.4)
    parts.append(e)
    e, t = line(f"   {GREEN}✓{RST} manifest.json + manifest.sig + manifest.cert.pem", t, 0.5)
    parts.append(e)
    e, t = line("", t, 0.2)
    parts.append(e)
    e, t = line(f"{GREEN}✓ Demo complete{RST} in 4.6s — DB wiped, no residue", t, 0.3)
    parts.append(e)
    e, t = line(f"  Output:", t, 0.2)
    parts.append(e)
    e, t = line(f"    {DIM}./demo-bundle/signed.pdf       (PAdES, 2 PKCS#7 signers){RST}", t, 0.2)
    parts.append(e)
    e, t = line(f"    {DIM}./demo-bundle/audit.json       (4 events, RFC 3161 anchor){RST}", t, 0.2)
    parts.append(e)
    e, t = line(f"    {DIM}./demo-bundle/manifest.{{json,sig,cert.pem}}{RST}", t, 1.5)
    parts.append(e)

    e, t = show(t, PROMPT)
    parts.append(e)
    e, t = keystroke("sign request verify-signed-pdf demo-bundle/signed.pdf", t, per_char=0.018)
    parts.append(e)
    e, t = line("", t, 0.4)
    parts.append(e)
    e, t = line(f"{GREEN}{{ ok: true, signatures: 2, chainValid: true, trust: \"self_signed_local\" }}{RST}", t, 1.5)
    parts.append(e)
